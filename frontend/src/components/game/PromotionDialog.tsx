import { useEffect, useCallback } from 'react';

interface PromotionDialogProps {
  isOpen: boolean;
  onSelect: (piece: 'q' | 'r' | 'b' | 'n') => void;
  playerColor: 'white' | 'black';
}

// SVG piece components matching standard chess piece style
const PieceIcon = ({ piece, color }: { piece: 'q' | 'r' | 'b' | 'n'; color: 'white' | 'black' }) => {
  const fill = color === 'white' ? '#f0f0f0' : '#2a2a2a';
  const stroke = color === 'white' ? '#2a2a2a' : '#f0f0f0';
  
  const pieces = {
    q: ( // Queen
      <svg viewBox="0 0 45 45" width="60" height="60">
        <g fill={fill} stroke={stroke} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
          <path d="M 9,26 C 17.5,24.5 30,24.5 36,26 L 38.5,13.5 L 31,25 L 30.7,10.9 L 25.5,24.5 L 22.5,10 L 19.5,24.5 L 14.3,10.9 L 14,25 L 6.5,13.5 L 9,26 z"/>
          <path d="M 9,26 C 9,28 10.5,28 11.5,30 C 12.5,31.5 12.5,31 12,33.5 C 10.5,34.5 11,36 11,36 C 9.5,37.5 11,38.5 11,38.5 C 17.5,39.5 27.5,39.5 34,38.5 C 34,38.5 35.5,37.5 34,36 C 34,36 34.5,34.5 33,33.5 C 32.5,31 32.5,31.5 33.5,30 C 34.5,28 36,28 36,26 C 27.5,24.5 17.5,24.5 9,26 z"/>
          <path d="M 11.5,30 C 15,29 30,29 33.5,30" fill="none"/>
          <path d="M 12,33.5 C 18,32.5 27,32.5 33,33.5" fill="none"/>
          <circle cx="6" cy="12" r="2"/>
          <circle cx="14" cy="9" r="2"/>
          <circle cx="22.5" cy="8" r="2"/>
          <circle cx="31" cy="9" r="2"/>
          <circle cx="39" cy="12" r="2"/>
        </g>
      </svg>
    ),
    r: ( // Rook
      <svg viewBox="0 0 45 45" width="60" height="60">
        <g fill={fill} stroke={stroke} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
          <path d="M 9,39 L 36,39 L 36,36 L 9,36 L 9,39 z"/>
          <path d="M 12,36 L 12,32 L 33,32 L 33,36 L 12,36 z"/>
          <path d="M 11,14 L 11,9 L 15,9 L 15,11 L 20,11 L 20,9 L 25,9 L 25,11 L 30,11 L 30,9 L 34,9 L 34,14"/>
          <path d="M 34,14 L 31,17 L 31,29.5 L 33,32 L 12,32 L 14,29.5 L 14,17 L 11,14"/>
          <path d="M 31,17 L 31,29.5" fill="none"/>
          <path d="M 14,17 L 14,29.5" fill="none"/>
        </g>
      </svg>
    ),
    b: ( // Bishop
      <svg viewBox="0 0 45 45" width="60" height="60">
        <g fill={fill} stroke={stroke} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
          <path d="M 9,36 C 12.39,35.03 19.11,36.43 22.5,34 C 25.89,36.43 32.61,35.03 36,36 C 36,36 37.65,36.54 39,38 C 38.32,38.97 37.35,38.99 36,38.5 C 32.61,37.53 25.89,38.96 22.5,37.5 C 19.11,38.96 12.39,37.53 9,38.5 C 7.65,38.99 6.68,38.97 6,38 C 7.35,36.54 9,36 9,36 z"/>
          <path d="M 15,32 C 17.5,34.5 27.5,34.5 30,32 C 30.5,30.5 30,30 30,30 C 30,27.5 27.5,26 27.5,26 C 33,24.5 33.5,14.5 22.5,10.5 C 11.5,14.5 12,24.5 17.5,26 C 17.5,26 15,27.5 15,30 C 15,30 14.5,30.5 15,32 z"/>
          <path d="M 25 8 A 2.5 2.5 0 1 1  20,8 A 2.5 2.5 0 1 1  25 8 z"/>
          <path d="M 17.5,26 L 27.5,26 M 15,30 L 30,30 M 22.5,15.5 L 22.5,20.5 M 20,18 L 25,18" fill="none" strokeLinecap="butt"/>
        </g>
      </svg>
    ),
    n: ( // Knight
      <svg viewBox="0 0 45 45" width="60" height="60">
        <g fill={fill} stroke={stroke} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
          <path d="M 22,10 C 32.5,11 38.5,18 38,39 L 15,39 C 15,30 25,32.5 23,18"/>
          <path d="M 24,18 C 24.38,20.91 18.45,25.37 16,27 C 13,29 13.18,31.34 11,31 C 9.958,30.06 12.41,27.96 11,28 C 10,28 11.19,29.23 10,30 C 9,30 5.997,31 6,26 C 6,24 12,14 12,14 C 12,14 13.89,12.1 14,10.5 C 13.27,9.506 13.5,8.5 13.5,7.5 C 14.5,6.5 16.5,10 16.5,10 L 18.5,10 C 18.5,10 19.28,8.008 21,7 C 22,7 22,10 22,10"/>
          <path d="M 9.5 25.5 A 0.5 0.5 0 1 1 8.5,25.5 A 0.5 0.5 0 1 1 9.5 25.5 z" fill={stroke} stroke={stroke}/>
          <path d="M 15 15.5 A 0.5 1.5 0 1 1  14,15.5 A 0.5 1.5 0 1 1  15 15.5 z" transform="matrix(0.866,0.5,-0.5,0.866,9.693,-5.173)" fill={stroke} stroke={stroke}/>
        </g>
      </svg>
    ),
  };
  
  return pieces[piece];
};

export const PromotionDialog = ({ isOpen, onSelect, playerColor }: PromotionDialogProps) => {
  const handleKeyDown = useCallback((e: KeyboardEvent) => {
    if (!isOpen) return;
    
    switch (e.key.toLowerCase()) {
      case 'q':
        onSelect('q');
        break;
      case 'r':
        onSelect('r');
        break;
      case 'b':
        onSelect('b');
        break;
      case 'n':
      case 'k': // Knight can also be selected with 'k'
        onSelect('n');
        break;
      case 'escape':
        // Default to queen on escape
        onSelect('q');
        break;
    }
  }, [isOpen, onSelect]);

  useEffect(() => {
    if (isOpen) {
      window.addEventListener('keydown', handleKeyDown);
      return () => window.removeEventListener('keydown', handleKeyDown);
    }
  }, [isOpen, handleKeyDown]);

  if (!isOpen) return null;

  const pieces: Array<{ type: 'q' | 'r' | 'b' | 'n'; name: string }> = [
    { type: 'q', name: 'Queen' },
    { type: 'r', name: 'Rook' },
    { type: 'b', name: 'Bishop' },
    { type: 'n', name: 'Knight' },
  ];

  return (
    <>
      {/* Backdrop */}
      <div
        style={{
          position: 'fixed',
          top: 0,
          left: 0,
          right: 0,
          bottom: 0,
          backgroundColor: 'rgba(0, 0, 0, 0.4)',
          display: 'flex',
          alignItems: 'center',
          justifyContent: 'center',
          zIndex: 9999,
        }}
        onClick={() => onSelect('q')} // Click outside defaults to queen
      />
      
      {/* Compact promotion selector */}
      <div
        style={{
          position: 'fixed',
          top: '50%',
          left: '50%',
          transform: 'translate(-50%, -50%)',
          backgroundColor: '#2a2a2a',
          borderRadius: '8px',
          boxShadow: '0 8px 32px rgba(0, 0, 0, 0.6)',
          padding: '0.5rem',
          display: 'flex',
          flexDirection: 'column',
          gap: '0.5rem',
          zIndex: 10000,
          border: '2px solid #4a4a4a',
        }}
        onClick={(e) => e.stopPropagation()}
      >
        {pieces.map((piece) => (
          <button
            key={piece.type}
            onClick={() => onSelect(piece.type)}
            title={piece.name}
            style={{
              width: '80px',
              height: '80px',
              backgroundColor: '#3a3a3a',
              color: '#e0e0e0',
              border: '2px solid #4a4a4a',
              borderRadius: '4px',
              cursor: 'pointer',
              display: 'flex',
              alignItems: 'center',
              justifyContent: 'center',
              transition: 'all 0.15s',
              padding: 0,
            }}
            onMouseEnter={(e) => {
              e.currentTarget.style.backgroundColor = '#b58863';
              e.currentTarget.style.borderColor = '#c69873';
              e.currentTarget.style.transform = 'scale(1.05)';
            }}
            onMouseLeave={(e) => {
              e.currentTarget.style.backgroundColor = '#3a3a3a';
              e.currentTarget.style.borderColor = '#4a4a4a';
              e.currentTarget.style.transform = 'scale(1)';
            }}
          >
            <PieceIcon piece={piece.type} color={playerColor} />
          </button>
        ))}
      </div>
    </>
  );
};
